...
public:
                     Task4(string name):Task(name)
     {
      ...
      group_mode = ORDER_GROUP_MODE_AUTOMATED;
      group_number = 25;
      int mtype[] = {1,2}; //0 for buy and 1 for sell
      ArrayCopy(type,mtype,0,0,WHOLE_ARRAY);
      count_limit = 0;
...
class Task5 : public Task
  {
   ...
   string            EachProfitMode;
   string            EachCompare;
   double            EachProfitAmount;
   double            EachProfitAmountPips;
   string            ProfitMode;
   string            Compare;
   double            ProfitAmount;
   double            ProfitAmountPips;
public:
                     Task5(string name):Task(name)
     {
      ...
      each_profit_mode = (string)"";
      each_compare = (string)">";
      each_profit_amount = (double)0.0;
      each_profit_amount_pips = (double)10.0;
      profit_mode = (string)"money";
      compare = (string)">";
      profit_amount = (double)0.0;
      profit_amount_pips = (double)10.0;
     }
   virtual void               run(int block_id, BlockParent &block)
     {
      Task::run(block_id, block);
      ...
      //+------------------------------------------------------------------+
      if(
         (ProfitMode == "money"    && (profitMoney >= ProfitAmount))
         || (ProfitMode == "pips"     && (profitPips <= ProfitAmountPips))
         || (ProfitMode == "pips-sum" && (pipsSum == ProfitAmountPips))
      )
        {
         printf("task" + block_id + " passed route 1");
         block.onResult(ROUTE_1_PASSED);
