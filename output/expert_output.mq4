...
bool DRAW_SPREAD_INFO   = false;int ticks_from_start    = 0;template <typename T>
   ...
   if(exp_date_str!="" && (TimeCurrent() > D'' || TimeLocal() > D''))
     {
      ...
      ExpertRemove();
     }addBlocksTick();addBlocksChart();addBlocksTrade();addBlocksTimer();addBlocksDeinit();	if (ENABLE_SPREAD_METER == false) {
		DRAW_SPREAD_INFO = false;
	}
	else {
		DRAW_SPREAD_INFO = !(MQLInfoInteger(MQL_TESTER) && !MQLInfoInteger(MQL_VISUAL_MODE));
	}
	if (DRAW_SPREAD_INFO) DrawSpreadInfo();if (ENABLE_STATUS) DrawStatus("waiting for tick...");	TesterHideIndicators(!ENABLE_TEST_INDICATORS);
...
void OnDeinit(const int reason){
   if(ENABLE_STATUS)
   DrawStatus("stopped");   if(ENABLE_SPREAD_METER)
      DrawSpreadInfo();
   EventKillTimer();
...
//__version__ = "0.9.28"
//__timestamp__ = "2024.10.20 13:32"
//